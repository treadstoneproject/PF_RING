			     ...
			     struct sk_buff *skb)
{
	 ...
	 */
	if (!fm10k_is_ies(dev) ||
	    fm10k_glort_valid_pf(&((struct fm10k_intfc *)netdev_priv(dev))->hw,
				 le16_to_cpu(FM10K_CB(skb)->fi.w.dglort)))
...
static inline bool fm10k_skb_is_vlan_tagged(struct sk_buff *skb)
{
	struct net_device *dev = skb->dev;

	return FM10K_CB(skb)->fi.w.vlan && (dev->features & NETIF_F_HW_VLAN_RX);
}
#endif
			      ...
			      struct sk_buff *skb)
{
	...
	if (fm10k_skb_is_vlan_tagged(skb)) {
		if (*vlgrp) {
			/* we have a vlgrp so we can receive on the VLAN */
			vlan_gro_receive(&q_vector->napi, *vlgrp,
					 le16_to_cpu(FM10K_CB(skb)->fi.w.vlan),
					 skb);
			return;
		} else if (FM10K_CB(skb)->fi.w.vlan !=
			   interface->hw.mac.default_vid) {
			/* this isn't the default VLAN ID, so drop it */
			dev_kfree_skb_any(skb);
			return;
		}
	}
...
#if defined(HAVE_SKB_INNER_PROTOCOL_TYPE)
	if (skb->inner_protocol_type != ENCAP_TYPE_ETHER ||
	    skb->inner_protocol != htons(ETH_P_TEB))
		return 0;

#elif defined(HAVE_SKB_INNER_PROTOCOL)
...
static u8 fm10k_tx_desc_flags(struct sk_buff *skb, u32 tx_flags)
{
	...
	/* set FTAG presence bits */
	if (skb->protocol == htons(ETH_P_IES))
		desc_flags |= FM10K_TXD_FLAG_FTAG;
			 ...
			 struct fm10k_tx_buffer *first)
{
	...
	/* Strip timestamp from outgoing IES frames */
	if (skb->protocol == htons(ETH_P_IES)) {
		size -= sizeof(__be64);
		data += sizeof(__be64);
	}
				  ...
				  struct fm10k_ring *tx_ring)
{
	...
	first->skb = skb;
	zlen = (skb->protocol == htons(ETH_P_IES)) ? FTAG_ZLEN : ETH_ZLEN;
	first->bytecount = max_t(unsigned int, skb->len, zlen);
	first->gso_segs = 1;
