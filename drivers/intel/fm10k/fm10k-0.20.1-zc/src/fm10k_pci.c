...
static int __devinit fm10k_sw_init(struct fm10k_intfc *interface,
...
#endif
{
	...
	/* configure basic options (physical functions only) */
	if (hw->mac.type == fm10k_mac_pf && fm10k_check_options(interface))
		return -EINVAL;
 ...
 **/
#ifdef HAVE_CONFIG_HOTPLUG
static int __devinit fm10k_probe(struct pci_dev *pdev,
				 const struct pci_device_id *ent)
#else
...
#endif
{
	...
	/* Try starting UIO interface, if it fails note it and continue */
	err = fm10k_uio_probe(interface);
	if (err)
 ...
 **/
#ifdef HAVE_CONFIG_HOTPLUG
static void __devexit fm10k_remove(struct pci_dev *pdev)
#else
...
#endif
{
	...
	/* disable UIO interface */
	fm10k_uio_remove(interface);
...
#else
static struct pci_error_handlers fm10k_err_handler = {
#endif
