...
static int fm10k_set_priv_flags(struct net_device *netdev, u32 priv_flags)
{
	...
	if (priv_flags & BIT(FM10K_PRV_FLAG_IES)) {
		/* we only support IES on PF */
		if (interface->hw.mac.type == fm10k_mac_vf)
			return -EINVAL;
...
static int fm10k_set_reta(struct net_device *netdev, const u32 *indir)
{
	...
	/* Unfortunately there is no way to determine whether this request is
...
static int fm10k_get_reta_legacy(struct net_device *netdev,
				 struct ethtool_rxfh_indir *indir)
{
	u32 size = fm10k_get_reta_size(netdev);

	/* indir->size should be RETA_SIZE or 0 all other values are invalid */
	if (size == indir->size)
		fm10k_get_reta(netdev, indir->ring_index);
	else if (indir->size)
		return -EINVAL;
...
static int fm10k_set_reta_legacy(struct net_device *netdev,
				 const struct ethtool_rxfh_indir *indir)
{
	if (indir->size != fm10k_get_reta_size(netdev))
		return -EINVAL;

	return fm10k_set_reta(netdev, indir->ring_index);
}
...
static inline int fm10k_set_feature(struct net_device *dev, u32 data,
				    u32 feature_list)
{
	if (data)
		dev->features |= feature_list;
	else
		dev->features &= ~feature_list;
...
static int fm10k_set_tso(struct net_device *dev, u32 data)
{
	return fm10k_set_feature(dev, data, NETIF_F_TSO | NETIF_F_TSO6);
}

static int fm10k_set_rx_csum(struct net_device *dev, u32 data)
{
	return fm10k_set_feature(dev, data, NETIF_F_RXCSUM);
}

static u32 fm10k_get_rx_csum(struct net_device *netdev)
{
	return !!(netdev->features & NETIF_F_RXCSUM);
}

#ifdef ETHTOOL_GFLAGS
static int fm10k_set_flags(struct net_device *netdev, u32 data)
{
	return ethtool_op_set_flags(netdev, data, ETH_FLAG_RXHASH);
}
...
static const struct ethtool_ops fm10k_ethtool_ops = {
	...
	.set_tso		= fm10k_set_tso,
	.set_rx_csum		= fm10k_set_rx_csum,
	.get_rx_csum		= fm10k_get_rx_csum,
#ifdef ETHTOOL_GFLAGS
	.get_flags              = ethtool_op_get_flags,
	.set_flags              = fm10k_set_flags,
#endif /* ETHTOOL_GFLAGS */
#endif /* HAVE_NDO_SET_FEATURES */
...
static const struct ethtool_ops_ext fm10k_ethtool_ops_ext = {
...
#else
	.get_rxfh_indir		= fm10k_get_reta,
	.set_rxfh_indir		= fm10k_set_reta,
#endif /* ETHTOOL_GRSSH */
#else
	.get_rxfh_indir		= fm10k_get_reta_legacy,
	.set_rxfh_indir		= fm10k_set_reta_legacy,
#endif /* HAVE_ETHTOOL_GRXFHINDIR_SIZE */
#endif /* ETHTOOL_GRXFHINDIR */
#ifdef ETHTOOL_SCHANNELS
...
#ifdef HAVE_RHEL6_ETHTOOL_OPS_EXT_STRUCT
	set_ethtool_ops_ext(dev, &fm10k_ethtool_ops_ext);
#endif
}
